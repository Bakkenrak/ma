(function() {
	...
	d2sApp.controller('headerController', function($scope, $rootScope,
		...
		$rootScope.$on('$locationChangeSuccess', function(event) {
			$scope.currentTab = $location.path().substr(1); // set scope
															// variable to
															// current route
	...
	d2sApp.controller('registrationCtrl', function($scope, $location,
			authFactory) {
		$scope.isAdmin = (authFactory.isAuthenticated() && authFactory
				.getAuthData().authRole === "admin");
		$scope.user = {};
		...
		$scope.register = function() {
			if ($scope.user.password === $scope.user.passwordConfirm) {
				...
				authFactory.register($scope.user).success(
	...
	d2sApp
			.controller(
					...
					function($scope, platforms) {
						...
						$scope
								.$watch(
										"pagination.currentPage + pagination.itemsPerPage + filteredPlatforms",
										function() {
											$scope.pagination.begin = (($scope.pagination.currentPage - 1) * $scope.pagination.itemsPerPage);
											$scope.pagination.end = $scope.pagination.begin
													+ $scope.pagination.itemsPerPage;

											// adjust end if it exceeds the
											// actual numer of platforms
											if ($scope.pagination.end > $scope.filteredPlatforms.length)
												$scope.pagination.end = $scope.filteredPlatforms.length;
											// slice the platforms for display
											// according to currently selected
											// page
											$scope.paginatedPlatforms = $scope.filteredPlatforms
													.slice(
															$scope.pagination.begin,
															$scope.pagination.end);
										});
										...
										function() {
											...
											angular
													.forEach(
															$scope.platforms,
															function(platform) {
																if (platform.label !== null
																		&& ~platform.label
																				.toLowerCase()
																				.indexOf(
																						$scope.searchTerm)) // add
																											// those
																											// whose
																											// label
																											// contains
											...
											$scope.pagination.currentPage = 1; // set
																				// pagination
																				// to
																				// first
																				// page
	...
	d2sApp.controller('platformDetailCtrl', function($scope, platformFactory,
			$rootScope, platform) {
		if (!angular.isUndefined(platform)) {
			...
			if (!angular.isUndefined($scope.platform.yearLaunch))
				$scope.platform.yearLaunchName = $scope.platform.yearLaunch
						.replace("http://www.discover2share.net/d2s-ont/", "");
		}
	...
	d2sApp
			.controller(
					'addPlatformCtrl',
					function($scope, $rootScope, platformFactory) {
						// retrieve dimension comments and labels (only once)
						if (angular.isUndefined($rootScope.descriptions)) {
							platformFactory.getDescriptions().success(
									function(data) {
										$rootScope.descriptions = data;
									});
						}
						if (angular.isUndefined($rootScope.countries)) {
							platformFactory.getCountries().success(
									function(data) {
										$rootScope.countries = data.countries;
									});
						}
						if (angular.isUndefined($rootScope.languages)) {
							platformFactory.getLanguages().success(
									function(data) {
						...
						$scope.residenceCountrySelected = function(item) {
							...
							$scope.platform.residenceCountry.geonames = "http://www.geonames.org/"
									+ item.countryId;
							$scope.platform.residenceCountry.label = item.countryName;
							if (!angular
									.isUndefined($scope.platform.residenceCity.item)
									&& $scope.platform.residenceCity.item.countryCode !== $scope.platform.residenceCountry.item.countryCode)
								console
										.log("Selected residence city is not situated in the selected country.");
						}
						...
						$scope.findLaunchCity = function() {
							...
							return platformFactory
									.findCity(
											$scope.platform.launchCity.search,
											$scope.platform.launchCountry.item.countryCode)
						...
						$scope.findResidenceCity = function() {
							...
							return platformFactory
									.findCity(
											$scope.platform.residenceCity.search,
											$scope.platform.residenceCountry.item.countryCode)
									.then(function(response) {
										return response.data.geonames;
									});
						};
						...
						$scope.getYears = function() {
							...
							var output = [];
							for (var i = currentYear; i > 1989; output
									.push(i--))
								;
							return output;
						};
						...
						$scope.marketMediations = [
								...
								"Profit from peer providers",
								"Profit from both", "Indirect profit",
								"Profit from advertisement",
								"Profit from user data" ];
						$scope.consumerisms = [ "Social", "Environmental",
								"Economic" ];
						$scope.smartphoneApps = [ "Android app", "iOS app",
								"Windows Phone app" ];
						$scope.trustContributions = [ "Provider ratings",
								"Provider and consumer ratings", "Referral",
								"Vouching", "Value-added services" ];
						...
						$scope.submit = function() {
							...
							$scope.platform.languages = $scope.platform.languages
									.filter(
											function(i) {
												return !angular
														.isUndefined(i.resourceName);
											}).map(function(i) {
										return i.resourceName;
									});
							platformFactory
									.addPlatformSuggestion($scope.platform);
						};
					});
